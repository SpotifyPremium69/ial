...
void List_Init( List *list ) {
    // Nastavení aby první a aktivní element ukazoval na NULL
...
void List_Dispose( List *list ) {
	ListElementPtr current = list->firstElement; // Začínáme od prvního prvku
    // Procházíme všechny prvky seznamu a uvolňujeme paměť pro aktuální prvek v pomocné proměnné temp
    while (current != NULL) {
        ListElementPtr temp = current;
...
void List_InsertFirst( List *list, int data ) {
	ListElementPtr newElement = (ListElementPtr)malloc(sizeof(ListElementPtr)); // Alokace paměti pro nový prvek
    if (newElement == NULL) {
        List_Error();
        return;
    }
    // Vložíme prvek na začátek seznamu
...
void List_GetFirst( List *list, int *dataPtr ) {
    // Pokud je seznam prázdný voláme Error
	if (list->firstElement == NULL) {
        List_Error();
    } else {
        // Jinak vrátíme hodnotu prvního prvku v seznamu
...
void List_DeleteFirst( List *list ) {
	if (list->firstElement != NULL) {
        ...
        // Posunutí prvku na první pozici a uvolnění místa
...
void List_DeleteAfter( List *list ) {
    // Zrušení prku za aktivním prvkem pokud je list aktivní a pokud není aktivní prvek poslední
...
void List_InsertAfter( List *list, int data ) {
	if (list->activeElement != NULL) {
        ...
        // Vložíme prvek za aktivní prvek
        newElement->data = data;
...
void List_GetValue( List *list, int *dataPtr ) {
	...
	if (list->activeElement == NULL) {
        ...
        // Jinak vrátíme hodnotu aktivního prvku v seznamu
...
void List_Next( List *list ) {
    // Posunutí aktivity na další prvek
	if (list->activeElement != NULL) {
